...
const utils = require('../../../test/cli/test-utils')
...
exports.handler = (argv) => {
  ...
  const put = `${utils.bin} docstore put ${dbname} "${testData.replace(/\"/g, '\\"')}" name`
  const search = `${utils.bin} docstore search ${dbname} "${argv.name}" -p`
